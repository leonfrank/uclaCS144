...
<%
		 ...
		 BidResult[] Bid_r = (BidResult[])request.getAttribute("Bid_Results");
		 String[] Category_r = IR.getCategories(); 
		 int XMLisEmpty = (Integer) request.getAttribute("EmptyItem");
%>
<!DOCTYPE html>
<html> 
<head>
		...
		function initialize() 
		{
			   ...
			   var address;
				...
				map = new google.maps.Map(document.getElementById("map"), mapOptions);
				 ...
				 if ( isEmpty(<%=IR.getLatitude()%>) || isEmpty(<%=IR.getLongitude()%>)) 
				 {
					 var Location = ('<%= IR.getLocation()%>').replace(/"/g, "");
					 var Country = ('<%= IR.getCountry() %>').replace(/"/g, "");
